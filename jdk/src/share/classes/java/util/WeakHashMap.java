...
public class WeakHashMap<K,V>
    ...
    final int hash(Object k) {
        ...
        // constant multiples at each bit position have a bounded 每个bit位的边界
        // number of collisions (approximately 8 at default load factor). 碰撞数量
        h ^= (h >>> 20) ^ (h >>> 12);
        return h ^ (h >>> 7) ^ (h >>> 4);
    }
    ...
    /**
     * Expunges(清除) stale entries from the table.
     */
    ...
    public V put(K key, V value) {
        ...
        modCount++;
        //
        Entry<K,V> e = tab[i]; //如果key不存在 必为null
        tab[i] = new Entry<>(k, value, queue, h, e/*null*/);
        if (++size >= threshold)
            resize(tab.length * 2);
        return null;
    }
